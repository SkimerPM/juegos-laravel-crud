...
<div class="mb-4">
    <label for="creador" class="block text-gray-700 text-sm font-bold mb-2">Creador / Compañía:</label>
    <input type="text" name="creador" id="creador" value="{{ old('creador', $plataforma->creador ?? '') }}"
           class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('creador') border-red-500 @enderror">
    @error('creador')
        <p class="text-red-500 text-xs italic">{{ $message }}</p>
    @enderror
</div>
